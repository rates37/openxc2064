...
const CellDisplay = React.memo(({ cell, tick, onSelect }: { cell: LogicCell; tick: number; onSelect: (cell: LogicCell) => void }) => {
    ...
    return (
        <g>
            {cell.nets
                .filter((net) => net.points.length > 0)
                .map((net, netIndex) => (
                    <LineSegment
                        key={netIndex}
                        value={net.value}
                        baseX={x + CELL_WIDTH / 2}
                        baseY={y + CELL_HEIGHT / 2}
                        points={net.points}
                    />
                ))}
            <rect
...
const MatrixDisplay = React.memo(
    ({ matrix, index, tick, onSelect }: { matrix: SwitchMatrix; index: number; tick: number; onSelect: (m: SwitchMatrix) => void }) => {
            ...
            <g>
                ...
                {lines.map(({ from, to }) => (
                    <line
                        ...
                        x1={cx + miniNodeOffsets[from].dx}
                        y1={cy + miniNodeOffsets[from].dy}
                        x2={cx + miniNodeOffsets[to].dx}
                        y2={cy + miniNodeOffsets[to].dy}
                        stroke="#2196F3"
                        strokeWidth={1.5}
                        pointerEvents="none"
                    />
                ...
                {miniNodeOffsets.map((off, i) => (
                    <circle key={`mn-${index}-${i}`} cx={cx + off.dx} cy={cy + off.dy} r={2} fill="#333" pointerEvents="none" />
                ))}
            </g>
        );
    }
...
const PipDisplay = React.memo(({ pip, index, onToggle }: { pip: Pip; index: number; onToggle: (i: number) => boolean }) => {
    ...
    return (
        <rect
            x={pip.pos.x - PIP_WIDTH / 2}
            y={pip.pos.y - PIP_HEIGHT / 2}
            width={PIP_WIDTH}
            height={PIP_HEIGHT}
        ...
        return (
            <g>
                <rect
                    x={x}
                    y={y}
                    width={bank.size.width}
                    height={bank.size.height}
                    fill={isActive ? "#f76420" : "#fff"}
                    stroke="#333"
                    strokeWidth={5}
                    rx={5}
                    ry={5}
